    ...
    def register_node(cls, node_cls: Type[BaseNode]):
        ...
        try:
            ...
            cls._node_metadata[node_type] = {
                'type': node_type,
                'description': inspect.getdoc(node_cls) or node_type, # 使用文档字符串作为描述
                'module': node_cls.__module__,
                'params_schema': schema,
                'inputs': {k: v.__name__ for k, v in inputs.items()}, # 存储类型名称
                'outputs': {k: v.__name__ for k, v in outputs.items()} # 存储类型名称
            }
            logger.info(f"成功注册节点类型: {node_type} 从 {node_cls.__module__}")
        except NotImplementedError:
             logger.error(f"节点类 {node_type} 未完全实现抽象方法 (如 get_params_schema, define_inputs, define_outputs)，无法存储完整元数据。")
        except Exception as e:
            logger.error(f"注册节点 {node_type} 时提取元数据失败: {e}")


        return node_cls # 返回原类，使装饰器可以链式调用或直接使用
