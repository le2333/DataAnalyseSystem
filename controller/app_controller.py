...
from controller.single_dim_controller import SingleDimController
# 导入服务，确保它们被加载和注册
import services.visualizers
import services.data_structuring # 导入新的数据结构化服务
# import services.preprocessors # 如果有预处理器的话
...
class AppController:
    ...
    def __init__(self):
        ...
        self.single_dim_controller = SingleDimController(data_manager=self.data_manager)
        # 在这里可以添加多维数据处理、分析、可视化的 Controller
        # self.multi_dim_controller = MultiDimController(data_manager=self.data_manager)
    ...
    def _build_layout(self) -> pn.layout.Tabs:
        ...
        tabs = pn.Tabs(
            ...
            ("一维处理与可视化", self.single_dim_controller.get_view_panel()),
