...
class TimeFrequencyPlotNode(BaseNode):
    ...
    def process(
        ...
        non_zero_min = (
            np.min(spectrogram[spectrogram > 0]) if np.any(spectrogram > 0) else 1e-10
        )
        ...
        # 返回图表数据
        return {"plot_type": "time_frequency", "figure": fig.to_json(), "title": title}
