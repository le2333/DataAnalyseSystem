...
pn.extension('param', 'plotly', 'tabulator') # 'plotly' 可能不是必需的，但通常与 hvplot 一起使用
...
class LinePlotNode(BaseNode):
    ...
    # 内部状态，用于存储输入数据的列名
    _input_columns = param.List(default=[], precedence=-1) # precedence=-1 使其不在UI中显示
    ...
    def _create_plot(self, df: pl.DataFrame, x_col: str, y_cols: list[str], title: str,
                       xlabel: str, ylabel: str, use_rasterize: bool, legend_position: str,
...
并确保输入数据已连接。""", halign='center', valign='center').opts(
        ...
        try:
            ...
            if use_rasterize:
                ...
                plot = plot.opts(hv.opts.Curve(rasterize=True), hv.opts.Scatter(rasterize=True))
...
检查列选择和数据类型。"""
    ...
    def _generate_preview_plot(self, x_col: str | None, y_cols: list[str], title: str,
                              xlabel: str, ylabel: str, use_rasterize: bool, legend_position: str,
                              width: int, height: int):
        """为配置面板生成预览图"""
        # --- 预览数据获取逻辑 ---
        ...
        if not self._input_columns: # 如果没有从输入获取到列名，创建一些通用列名
            ...
            for col in self._input_columns:
                 ...
                 if 'time' in col.lower() or 'date' in col.lower():
                     data[col] = pl.datetime_range(start=datetime(2024,1,1), end=datetime(2024,1,2), interval=f"{86400//n_rows}s", eager=True)[:n_rows]
